...
class MulticlassifyLoss(nn.Module):

    def __init__(self):
        super(MulticlassifyLoss, self).__init__()

    def forward(self, preds, targets):
        n_classes = preds.shape[1]
        targets = targets.contiguous()
        targets = F.one_hot(targets.to(torch.int64), n_classes).squeeze(1)
        preds = preds.permute(0, 2, 3, 1)
        dims = tuple(range(1, targets.ndimension() - 1))
...
class MultiMetric(Metric):
    ...
    def __init__(self, output_transform=lambda x: x, device="cpu"):
        ...
        super(MultiMetric, self).__init__(output_transform=output_transform, device=device)
    ...
    def update(self, output):
        y_pred, y = output[0].detach().cpu(), output[1].detach().cpu()
